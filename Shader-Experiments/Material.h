...
struct Material {
  ...
  // Unbinds shader
  void unbindMaterial() { glUseProgram(0); }

  // Get the shader's internal ID number
  unsigned int getID() { return shad.programHandle; }

  // Check to see if this material has been flagged as alphablended
  bool isAlphaBlended() { return bAlphaBlended; }

  // Checks to see if this material is set to use the time variable
  bool usesTimeVariable() { return bUseTimeVariable; }

  // Updates the time of the shader
  void updateTime(float current_time) { currentTime = current_time; }
