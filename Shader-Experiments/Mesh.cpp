...
Mesh::Mesh(string file_name, string Name, Material *theMaterial, float X,
           float Y, float Z, float Xscale, float Yscale, float Zscale,
...
void Mesh::drawMesh(bool renderOnlyIfOpaque, bool renderOnlyIfTranslucent) {
  if (material != NULL) {
    if (material->isAlphaBlended() && renderOnlyIfOpaque)
      return;
    if (!material->isAlphaBlended() && renderOnlyIfTranslucent)
      return;
  }
...
bool Mesh::parseMeshFile(string filename) {
  ...
  if (!fin) {
    cout << "WARNING! Mesh file \"" << filename << "\" could not be found"
         << endl;
    fin.close();
    return false;
  }
  ...
  while (!fin.eof()) {
    ...
    if (firstToken[0] == '#')
      continue; // i.e. check for a "comment" line
