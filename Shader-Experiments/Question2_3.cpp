...
void initializeScene() {
  ...
  Material *airplaneMaterial = new Material(
      "student_code/diffuseTexture.vert", "student_code/diffuseTexture.frag",
      false, false, airplaneTexture->getTextureID());

  Material *waterMaterial = new Material("student_code/oceanWater.vert",
                                         "student_code/oceanWater.frag", true,
                                         true, waterTexture->getTextureID());
  timeBasedMaterials.push_back(waterMaterial);

  Material *skyMaterial =
      new Material("student_code/skyShader.vert", "student_code/skyShader.frag",
                   false, false, skyTexture->getTextureID());

  Material *sunMaterial = new Material("student_code/theSun.vert",
                                       "student_code/theSun.frag", true, false);

  Material *islandSandMaterial = new Material(
      "student_code/islandTerrain.vert", "student_code/islandTerrain.frag",
  ...
  Material *lightMappedMetalSheetMaterial = new Material(
      ...
      "student_code/lightmappedMetal.frag", false, true,
      metalSheetTexture->getTextureID(), lightMapTexture->getTextureID());
  ...
  Material *radioSpindlesMaterial = new Material(
      "student_code/tapeSpindles.vert", "student_code/tapeSpindles.frag", false,
      true, rustySpindles->getTextureID());
  timeBasedMaterials.push_back(radioSpindlesMaterial);

  Material *radioBeaconMaterial =
      new Material("student_code/beaconLight.vert",
                   "student_code/beaconLight.frag", false, true);
  timeBasedMaterials.push_back(radioBeaconMaterial);

  Material *radioButtonsMaterial = new Material(
      "student_code/buttons.vert", "student_code/buttons.frag", false, false);
  ...
  Mesh *seafloorMesh =
      new Mesh("Meshes/waterSurface.obj", "Sea floor", shallowSandMaterial, 0,
               0, -500, 1500.0, 1500.0, 1.0, 0, 90);
  meshes.push_back(seafloorMesh);

  Mesh *skyMesh = new Mesh("Meshes/skyDome.obj", "Sky", skyMaterial, 0, 0, -500,
                           1000, 1000, 1000);
  meshes.push_back(skyMesh);

  Mesh *sandyIslandMesh =
      new Mesh("Meshes/sandyIsland.obj", "Sandy Island", islandSandMaterial);
  meshes.push_back(sandyIslandMesh);

  Mesh *blackRocksMesh =
      new Mesh("Meshes/blackRocks.obj", "Black Rocks", blackRockMaterial);
  meshes.push_back(blackRocksMesh);

  Mesh *palmTreeTrunksMesh =
      new Mesh("Meshes/treeTrunks.obj", "Palm Tree Trunks", palmBarkMaterial);
  meshes.push_back(palmTreeTrunksMesh);

  Mesh *palmTreeTopsMesh =
      new Mesh("Meshes/treeTops.obj", "Palm Tree Tops", frawnMaterial);
  meshes.push_back(palmTreeTopsMesh);

  Mesh *smallSpikeyPlantsMesh =
      new Mesh("Meshes/smallPlants.obj", "Spikey Plants", frawnMaterial);
  meshes.push_back(smallSpikeyPlantsMesh);

  Mesh *underwaterSlopeMesh =
      new Mesh("Meshes/underwaterSlope.obj", "Shallow Sandy Depths",
               shallowSandMaterial);
  meshes.push_back(underwaterSlopeMesh);

  Mesh *airplaneMesh =
      new Mesh("Meshes/airplane.obj", "Crashed Airplane", airplaneMaterial,
               2500, -1400, 0, 90, 90, 90, -60, 150, 30);
  meshes.push_back(airplaneMesh);

  Mesh *metalSheetOnBeachMesh =
      new Mesh("Meshes/metalSheetOnBeach.obj", "Metal Sheet on Beach",
               metalSheetMaterial);
  meshes.push_back(metalSheetOnBeachMesh);

  Mesh *metalSheetByRadioMesh =
      new Mesh("Meshes/metalSheetByRadio.obj", "Metal Sheet by Radio",
               lightMappedMetalSheetMaterial);
  meshes.push_back(metalSheetByRadioMesh);

  Mesh *bonyHandMesh =
      new Mesh("Meshes/bonyHand.obj", "Bony Hand", brightBoneMaterial);
  meshes.push_back(bonyHandMesh);

  Mesh *radioBoxMesh =
      new Mesh("Meshes/radioBox.obj", "Radio Box", radioBoxMaterial);
  meshes.push_back(radioBoxMesh);

  Mesh *radioDetailsMesh = new Mesh("Meshes/radioDetails.obj", "Radio Details",
                                    radioDetailsMaterial);
  ...
  Mesh *radioButtonsMesh = new Mesh("Meshes/radioButtons.obj", "Radio Buttons",
                                    radioButtonsMaterial);
  meshes.push_back(radioButtonsMesh);

  sunMesh =
      new Mesh("Meshes/eyeBall.obj", "The sun", sunMaterial, sun.position[0],
               sun.position[1], sun.position[2], 600.0, 600.0, 600.0);
